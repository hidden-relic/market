...
function DisplayWelcomeGroundTextAtSpawn(player, pos)
    ...
    -- Render some welcoming text...
    local tcolor = {0.3, 0.9, 0.3, 0.4}
    local ttl = 2000
    local rid1 = rendering.draw_text{text="Good",
    ...
    local rid2 = rendering.draw_text{text="Luck!",
                        surface=game.surfaces[GAME_SURFACE_NAME],
...
function QueuePlayerForDelayedSpawn(playerName, spawn, moatEnabled, vanillaSpawn)
    ...
    if ((spawn.x ~= 0) or (spawn.y ~= 0)) then
        ...
        local delay_spawn_seconds = 2*(math.ceil(global.ocfg.spawn_config.gen_settings.land_area_tiles/CHUNK_SIZE))
